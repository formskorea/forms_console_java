...
class AWorkController {
        ...
        if (rtnValue.status == DefaultConfig.SERVER_SUCCESS) {
            try {
                ...
                if (result == null) {
                    ...
                    countService.createWorkLog(data)
    ...
    @ResponseBody
    @Throws(Exception::class)
    fun editWork(@RequestBody data: Work, response: HttpServletResponse, request: HttpServletRequest): Any {
        val rtnValue = ReturnValue()
        ...
        if (rtnValue.status == DefaultConfig.SERVER_SUCCESS) {
            try {
                var result = adminService.editMWork(data)
